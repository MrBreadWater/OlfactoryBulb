"""Smoke tests for the declarative validation-design-review coverage audit."""
...
report = run_validation_design_review_status(argparse.Namespace())
item_by_id = {item.check_id: item for item in report.items}

assert report.audit_id == "validation_design_review_status"
...
assert item_by_id["reference_validation_pending_items"].status == "WARN"
...
cli = subprocess.run(
    [sys.executable, "tools/run_audit.py", "validation_design_review_status", "--json"],
    capture_output=True,
    text=True,
    check=False,
)
...
assert payload["audit_id"] == "validation_design_review_status"
assert payload["summary"]["FAIL"] == 0
assert payload["summary"]["WARN"] >= 1

print("audit_validation_design_review_status: OK")
