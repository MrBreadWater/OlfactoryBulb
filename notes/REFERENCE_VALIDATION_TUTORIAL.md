...
4. Has a human reviewed the validation-design choice behind the comparison?
   - If no: mark it `pending_review`.
   - If yes, but it is still a temporary stopgap: mark it `provisional`.
   - If yes, and the design choice is accepted: mark it `accepted`.
...
human_review_note = "Protocol equivalence manually reviewed."
```
...
property_human_review_statuses = {
  "Membrane Resting Voltage" = "accepted",
...
- promote stable, justified choices to `accepted`
...
### `human_review_status`, `human_review_reviewer`, `human_review_note`
...
### `property_human_review_statuses`, `property_human_review_notes`, and `property_human_review_reviewers`
...
3. the item resolves to `pending_review` or `provisional`, which adds a
...
5. Use `pending_review` by default for new scientific design choices.
6. Use `provisional` only for acknowledged stopgaps.
7. Use `accepted` only after human review.
...
11. Run `python tools/run_audit.py human_review_status`.
