...
- Added reusable PSD-template plotting helpers, `psd_template_curve()` and `scaled_psd_template_curve()`, so diagnostics can overlay the same theoretical PSD shapes used by the v6 objective. Regenerated the C01123 packet's `01_psd_control.png`, `01_psd_ketamine.png`, `03_psd_overlay.png`, `contact_sheet.png`, and `manifest.json` with condition-specific target PSD overlays.
