...
- High-gamma visibility update: the optimizer already scored a `high_gamma = 65-100 Hz` band, but it was not prominent in the live best-candidate summaries or PSD packets. Added compact status summaries that report `control_high_gamma_rel` and `ketamine_high_gamma_rel`, and regenerated PSD overlays now mark the high-gamma band separately from the target HFO band.
- Runtime reduction update: future HFO optimizer batches now default to `tstop_ms = 1000` rather than `9000`. The mid-run ketamine-switch helper scales to a `500 ms` switch time and `100 ms` washout for 1 s simulations, preserving roughly `400 ms` control and `400 ms` ketamine scoring windows instead of carrying over the old 500 ms washout that would erase the ketamine window.
- Runtime objective pivot: after verifying batch 77 used the 1 s protocol, updated the campaign-local `objective_filter.json` to `min_batch_index = 77`. This keeps future ranking/proposal comparisons within the 1 s regime instead of mixing the old 9 s archive scores with the shorter-window scores. Batch 78 had already been planned from the old frontier pair before the filter took effect; batch 79+ should use the filtered short-run archive.
...
- Dashboard setup: added `tools/analysis/hfo_tensorboard_dashboard.py`, a sidecar TensorBoard exporter for campaign archives. It logs pair scores, best-so-far traces, condition metrics, target/high-gamma/beta/gamma band powers, spike rates, penalties, and parameter values from `candidate_archive.jsonl`; it also writes `top_candidates.csv` and can embed existing candidate-packet contact sheets. Event files live under `tensorboard/events/` so watcher/server logs and PID files can live in `tensorboard/` without being removed by a reset. This stays outside the simulation loop so Phoenix jobs do not depend on TensorBoard or TensorFlow.
- Local dashboard runtime: installed standalone `tensorboard` into the OBGPU environment and pinned `setuptools<81` because TensorBoard 2.20 still imports `pkg_resources`. Started a watcher sidecar that refreshes the active campaign event logs and a local TensorBoard server at `http://127.0.0.1:6006/`.
- Scoring correction after short-run batches 84/85: `C01358` became the numerical leader even though both control and ketamine EPLI rates were `0 Hz`, so the old `2 point` EPLI silence penalty was not strong enough to enforce the intended fast-interneuron-supported mechanism. Bumped `PAIR_SCORE_VERSION` to `7`, raised the ketamine EPLI silence penalty to `8 points`, and added a softer low-support penalty below `5 Hz`. This should stop the optimizer from treating silent-EPLI target-band LFP structure as a strong mechanistic candidate.

Throughput follow-up after the short-run pivot:

- The next bottleneck audit focused on low observed occupancy during concurrent Phoenix sweep items and a noticeable batch-end pause before the next optimizer wave started.
- Remote sweep step fix: `tools/remote/remote_sweep_driver.py` now injects `--exact` alongside `--exclusive` for concurrent child `srun` steps. The intent is to make each child step reserve only its requested `-n` ranks instead of starving sibling steps by grabbing excess allocation resources.
- Remote sweep tail-overlap fix: `obgpu_experiment_helpers.py` now begins compact finished-item sync automatically once a sweep reaches its final wave (`pending_labels == 0` while items are still running), even when full live sync is disabled. This should shift part of the final transfer cost out of the dead time between optimizer batches without changing normal notebook single-run behavior.
