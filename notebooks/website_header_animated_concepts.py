...
def add_density_ribbons(
    ...
    *,
    ...
    for cell_type in ("MC", "TC", "GC"):
        density = gaussian_smooth(summary.densities[cell_type], sigma_bins=2.0)
        ...
        ax.plot(x, y, color=color, lw=7.5, alpha=0.08, zorder=3)
        ax.plot(x, y, color=color, lw=2.2, alpha=0.98, zorder=4)
...
def render_aurora_currents(
    ...
    *,
    ...
    add_density_ribbons(
        ...
        amplitudes={"MC": 0.17, "TC": 0.21, "GC": 0.18},
        colors={"MC": SAND, "TC": SEA, "GC": MINT},
        fill_alpha=0.20,
    )

    x = np.linspace(0.34, 0.97, summary.x_norm.size)
    lfp_x = np.linspace(0.34, 0.97, summary.lfp.size)
    lfp_y = 0.08 + 0.025 * gaussian_smooth(summary.lfp, sigma_bins=2.0)
    ax.plot(lfp_x, lfp_y, color=INK, lw=1.4, alpha=0.48, zorder=5)

    flare_x = 0.36 + 0.56 * ((phase * 0.61) % 1.0)
...
def render_dendritic_pulse(
    ...
    *,
    ...
    combo = 0.45 * summary.densities["MC"] + 0.70 * summary.densities["TC"] + 0.60 * summary.densities["GC"]
    wave = 0.12 + 0.18 * gaussian_smooth(combo, sigma_bins=3.0)
    ...
    wave2 = 0.30 + 0.10 * gaussian_smooth(summary.densities["TC"], sigma_bins=2.0)
    ax.fill_between(wave_x, 0.28, wave2, color="#133947", alpha=0.18, zorder=1)
    ax.plot(wave_x, wave2, color=MINT, lw=1.6, alpha=0.34, zorder=2)

    pulse_centers = np.linspace(0.52, 0.90, 4)
    for pulse_idx, center in enumerate(pulse_centers):
        pulse = 0.04 + 0.06 * summary.active_score + 0.015 * math.sin((phase * 2 * math.pi) + pulse_idx)
        pulse_y = 0.47 + 0.04 * math.cos(phase * 2 * math.pi + pulse_idx)
    ...
    for shape in shapes:
        ...
        for x0, y0, x1, y1 in projected:
            ax.plot([x0, x1], [y0, y1], color=color, lw=shape.width * (1.6 + strength * 2.4), alpha=0.04 + strength * 0.03, zorder=3, solid_capstyle="round")
        for x0, y0, x1, y1 in projected:
            ax.plot([x0, x1], [y0, y1], color="#edf4f7", lw=shape.width * 0.95, alpha=0.16 + strength * 0.30, zorder=4, solid_capstyle="round")
            ax.plot([x0, x1], [y0, y1], color=color, lw=shape.width * 0.42, alpha=0.65 + strength * 0.25, zorder=5, solid_capstyle="round")
...
def render_spike_bloom(
    ...
    *,
    ...
    for cell_type in ("MC", "TC", "GC"):
        density = gaussian_smooth(summary.densities[cell_type], sigma_bins=1.5)
        ...
        amp = density[x_idx]
        keep = rng.random(count) < np.clip(amp * (1.9 if cell_type != "GC" else 1.5), 0.02, 0.98)
        x_pts = x_seed[keep]
        amp = amp[keep]
        drift = 0.012 * np.sin((phase * 2 * math.pi) + (x_pts * 8.0))
        y_pts = bands[cell_type] + drift + rng.normal(scale=0.02 + (0.015 * (1.0 - amp)), size=x_pts.size)
        sizes = 8 + (28 * amp)
        color = colors[cell_type]
        ax.scatter(x_pts, y_pts, s=sizes * 7.0, color=color, alpha=0.025, linewidths=0, zorder=2)
        ax.scatter(x_pts, y_pts, s=sizes, color=color, alpha=0.72, linewidths=0, zorder=4)
        streak_len = 0.01 + (0.015 * amp)
        for xp, yp, sl, strength in zip(x_pts[::5], y_pts[::5], streak_len[::5], amp[::5]):
            ax.plot([xp - sl * 0.4, xp + sl], [yp - sl * 0.5, yp + sl * 0.5], color=color, lw=0.6 + strength * 1.2, alpha=0.18 + strength * 0.40, zorder=3)

        ridge = bands[cell_type] - 0.06 + 0.12 * density
        ax.plot(x_base, ridge, color=color, lw=1.4, alpha=0.28, zorder=1)
...
def render_spectral_veil(
    ...
    *,
    ...
    rgb = rgb * field[..., None]
    alpha = np.clip(field ** 1.25, 0.0, 0.92)
    ...
    for level_idx, spec_idx in enumerate(np.linspace(2, spec.shape[0] - 3, 8).astype(int)):
        ...
        y = baseline + 0.025 * row
        color = [MINT, SEA, SAND][min(2, level_idx // 3)]
        ax.plot(x_small, y, color=color, lw=1.2 + 0.2 * level_idx, alpha=0.10 + 0.04 * level_idx, zorder=2)
...
def export_all_concepts(
    ...
    *,
    ...
    for concept_name in selected:
        ...
        posters[concept_name] = poster_path
