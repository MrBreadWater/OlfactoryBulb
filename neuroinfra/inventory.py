...
EXTRACTION_CANDIDATES: tuple[ExtractionCandidate, ...] = (
    ...
    ExtractionCandidate(
        ...
        proposed_phase=2,
        current_status="local_seams_standardized",
        recommended_action="The helper-bundle packaging and local command-building seams are now standardized under neuroinfra; next extract shared launcher logic without breaking the live remote entrypoints.",
    ),
    ExtractionCandidate(
    ...
    ExtractionCandidate(
        ...
        proposed_phase=5,
        current_status="candidate",
        recommended_action="Keep packet schema pluggable and extract only the runtime supervision and manifest protocol first.",
    ),
    ExtractionCandidate(
