"""Remote-safe submit helpers for one timestamped Slurm benchmark run."""
...
from neuroinfra.remote_script_common import (
    path_is_within,
...
def write_batch_script(
    ...
    lines = [
        ...
        *slurm_directives(args, label),
        ...
        "  local index_lock lock_mtime now age",
        "  index_lock=\"$shared_repo_root/.git/index.lock\"",
        "  [[ -f \"$index_lock\" ]] || return 0",
        "  if command -v lsof >/dev/null 2>&1 && lsof \"$index_lock\" >/dev/null 2>&1; then",
        "    printf '[OBGPU batch] git index.lock is still held by a live process; leaving it in place\\n' >> \"$bootstrap_log\"",
        "    return 1",
        "  fi",
        "  if command -v fuser >/dev/null 2>&1 && fuser \"$index_lock\" >/dev/null 2>&1; then",
        "    printf '[OBGPU batch] git index.lock is still owned according to fuser; leaving it in place\\n' >> \"$bootstrap_log\"",
        "    return 1",
        "  fi",
        ...
        "  age=$((now - lock_mtime))",
        "  if [[ \"$age\" -lt 30 ]]; then",
        "    printf '[OBGPU batch] git index.lock exists but is only %ss old; refusing stale-lock cleanup\\n' \"$age\" >> \"$bootstrap_log\"",
        "    return 1",
        "  fi",
        "  rm -f \"$index_lock\"",
        "  printf '[OBGPU batch] removed stale git index.lock (%ss old)\\n' \"$age\" >> \"$bootstrap_log\"",
        "}",
        "run_git_locked() {",
        "  local rc=0",
        "  mkdir -p \"$(dirname \"$git_lock_path\")\"",
        "  if command -v flock >/dev/null 2>&1; then",
        "    exec 9>>\"$git_lock_path\"",
        "    flock 9",
        ...
        "  trap - ERR",
        "  set +e",
        "  stop_notebook_heartbeat_watchdog || true",
        "  printf '[OBGPU batch] failed at line %s: %s (exit %s)\\n' \"$failed_line\" \"$failed_command\" \"$rc\" >> \"$bootstrap_log\"",
        "  sync_wrapper_artifacts",
        "  exit \"$rc\"",
        "}",
        "cleanup_worktree() {",
        ...
        "  set +e",
        "  stop_notebook_heartbeat_watchdog || true",
        "  sync_wrapper_artifacts || true",
        "  restore_shared_repo || true",
